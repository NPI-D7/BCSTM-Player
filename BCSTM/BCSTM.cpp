...
BCSTM::BCSTM() : m_isStreaming(false), m_isPaused(false), m_channelCount(0) {}

BCSTM::~BCSTM() { stop(); }

bool BCSTM::openFromFile(std::string filename) {
  stop();
  ...
  m_blockLoopEnd =
      (loopEnd % m_blockSampleCount ? m_blockCount
                                    : loopEnd / m_blockSampleCount);
  ...
  for (unsigned int i = 0; i < m_channelCount; i++) {
    fread(m_adpcmCoefs[i], 1, static_cast<size_t>(sizeof(uint16_t) * 16),
          m_file);
    fread(&m_adpcmData[i][0], 1, static_cast<size_t>(sizeof(ndspAdpcmData)),
          m_file); // Beginning Context
    fread(&m_adpcmData[i][1], 1, static_cast<size_t>(sizeof(ndspAdpcmData)),
          m_file); // Loop context
    read16();      // skip padding
  }
...
void BCSTM::tick() { streamData(); }
void BCSTM::streamData() {
...
void BCSTM::fillBuffers() {
  for (unsigned int bufIndex = 0; bufIndex < BufferCount; ++bufIndex) {
    ...
    if (m_looping && m_currentBlock == m_blockLoopEnd) {
      ...
      fseek(
          m_file,
          static_cast<size_t>(m_dataOffset + 0x20 +
                              m_blockSize * m_channelCount * m_blockLoopStart),
          SEEK_SET);
    }
    if (!m_looping && m_currentBlock == m_blockLoopEnd) {
      this->stop();
    ...
    for (unsigned int channelIndex = 0; channelIndex < m_channelCount;
         ++channelIndex) {
      ...
      fread(buf->data_adpcm, 1,
            static_cast<size_t>((m_currentBlock == m_blockCount - 1)
                                    ? m_lastBlockSize
                                    : m_blockSize),
            m_file);
...
bool BCSTM::fileAdvance(uint64_t byteSize) {
  uint64_t seekPosition = ftell(m_file) + byteSize;
  return ((unsigned)fseek(m_file, static_cast<size_t>(seekPosition),
                          SEEK_SET) == seekPosition);
}
