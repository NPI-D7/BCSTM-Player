...
template <typename T> class LinearAllocator : public std::__allocator_base<T> {
public:
  ...
  template <typename T1> LinearAllocator(const LinearAllocator<T1> &) throw() {}
  ...
  void deallocate(pointer __p, size_type) { linearFree(__p); }
...
/// container (e.g. std::vector) that allocates everything using ctrulib's
/// linear memory.
///
